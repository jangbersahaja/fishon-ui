...
const BADGE_LOOKUP = REVIEW_BADGES.reduce((acc, badge) => {
    acc[badge.id] = badge;
    return acc;
}, {});
...
export function getRandomBadges(seed, minimum = 2, maximum = 4) {
    const rng = createRng(seed);
    const pool = [...REVIEW_BADGES];
    const result = [];
    const count = Math.min(maximum, Math.max(minimum, Math.floor(rng() * (maximum - minimum + 1)) + minimum));
...
export function summariseBadges(reviews) {
    const tally = new Map();
    reviews.forEach((review) => {
        var _a;
        (_a = review.badges) === null || _a === void 0 ? void 0 : _a.forEach((id) => {
